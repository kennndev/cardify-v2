...
export default function AvatarUploader({ uid, initialUrl, onUpdated, size = 96 }: Props) {
  ...
  const onFile = async (file?: File) => {
    ...
    try {
      ...
      await supabase.from("mkt_profiles").upsert({ id: uid, avatar_url: publicUrl }, { onConflict: "id" })
  ...
  const removeAvatar = async () => {
    ...
    try {
      ...
      await supabase.from("mkt_profiles").upsert({ id: uid, avatar_url: null }, { onConflict: "id" })
