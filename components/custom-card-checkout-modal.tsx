...
export function CustomCardCheckoutModal({ isOpen, onClose, uploadedImage, processedImageBlob, uploadedImageUrl }: CustomCardCheckoutModalProps) {
  ...
  const handleAddToCart = async () => {
    ...
    // Use the pre-uploaded URL if available, otherwise use the local image
    const imageUrl = uploadedImageUrl || uploadedImage
                        ...
                        <div className="text-xs text-gray-400 bg-cyber-dark/40 rounded-lg p-2 space-y-1">
                          ...
                          {inventoryState.data.customCard.pricingTiers.slice(1).map((tier, index) => (
                            <div key={index} className="flex justify-between">
                              ...
                              <span className="text-cyber-purple">{tier.discount}% off</span>
                            </div>
                          ))}
                        </div>
                      )}
