...
import { Pencil, Check, X, Sparkles, Trash2, Loader2, AlertTriangle, ChevronDown, Upload, Plus, Package } from "lucide-react"
              ...
              <div className="flex items-center gap-2">
                <button
                  className={`min-h-[40px] py-2 rounded border border-cyber-cyan/30 bg-cyber-dark/60 text-white flex items-center transition-colors ${
                    !displayName && !nameLoading ? "px-4 gap-2 justify-center hover:border-cyber-cyan/50 hover:bg-cyber-dark/80 cursor-pointer" : "min-w-[12rem] px-3 cursor-default"
                  }`}
                  onClick={() => {
    ...
    <Card className="bg-cyber-dark/60 border border-cyber-cyan/30">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          {/* Create button - aligned left on mobile, inline on desktop */}
          {uid && !loadingAuth && (
              ...
              <div key={i} className="bg-cyber-dark/60 border border-cyber-cyan/30 rounded-lg p-3 animate-pulse">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex gap-2">
                    <div className="flex-1 h-8 bg-cyber-cyan/10 rounded animate-pulse" />
                    <div className="w-8 h-8 bg-cyber-cyan/10 rounded animate-pulse" />
                  </div>
                  ...
                  >
                    <CardContent className="p-3">
                      ...
                      >
                        ...
                        {/* Hover overlay with badges and view text */}
                        <div className="absolute inset-0 pointer-events-none">
                          ...
                          {/* Badges container with staggered animation - all badges commented out but preserved */}
                          <div className="absolute top-3 left-3 right-3 flex flex-wrap gap-2">
                            {/* Uploaded badge - commented out but preserved for future use
                            <Badge className="bg-cyber-cyan/20 border border-cyber-cyan/40 text-cyber-cyan text-xs opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                              Uploaded
                            </Badge>
                            */}
                            {/* Listed badge - commented out but preserved for future use
                            {listed && (
                              <Badge className="bg-green-500/20 border border-green-500/40 text-green-400 text-xs opacity-0 group-hover:opacity-100 transition-all duration-300 delay-75 transform translate-y-2 group-hover:translate-y-0">
                                Listed
                              </Badge>
                            )}
                            */}
                            {/* File type badge - commented out but preserved for future use
                            {a.mime_type && (
                              <Badge className="bg-cyber-pink/20 border border-cyber-pink/40 text-cyber-pink text-xs opacity-0 group-hover:opacity-100 transition-all duration-300 delay-150 transform translate-y-2 group-hover:translate-y-0">
      ...
      <DialogContent className="bg-cyber-dark/95 border-2 border-cyber-cyan/50 text-white">
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-300 break-words">
            ...
            <span className="text-white font-semibold">{selectedAsset?.file_name}</span>
          </div>
